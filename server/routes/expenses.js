...
router.post('/', async (req, res) => {
  try {
    ...
    const expense = await Expense.create({
      user: req.user._id, item, amount, category, date,
...
router.get('/export/csv', async (req, res) => {
  try {
    ...
    const rows = [
      ...
      ...expenses.map(e => [
        e.date, esc(e.item), e.category, e.amount.toFixed(2), esc(e.notes),
...
router.get('/export/pdf', async (req, res) => {
  try {
    ...
    doc.text('Date', 40, y);  doc.text('Description', 105, y);
    doc.text('Category', 310, y); doc.text('Amount', 460, y, { width: 95, align: 'right' });
    y += 14;
    doc.moveTo(40, y).lineTo(555, y).strokeColor('#2e3250').stroke();
    y += 6;
    ...
    expenses.forEach((e, idx) => {
      ...
      doc.fillColor('#ff6584').font('Helvetica-Bold')
        .text(`£${e.amount.toFixed(2)}`, 460, y, { width: 93, align: 'right' });
      if (e.items && e.items.length > 0) {
        y += 14;
        const detail = e.items.map(i => `${i.name} ×${i.qty} @£${i.price.toFixed(2)}`).join('   ');
        doc.fillColor('#6c63ff').fontSize(7).font('Helvetica')
          .text('  → ' + detail, 115, y, { width: 430 });
    ...
    doc.fontSize(8).fillColor('#8b90b8')
      .text('SpendSmart — Your personal expense tracker', 40, 800, { align: 'center', width: 515 });
    doc.end();
  } catch (err) {
